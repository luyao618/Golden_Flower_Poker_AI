...
- [x] 用 `<img>` 替换 CSS 椭圆渐变绘制的牌桌
- [x] 图片设为 `object-fit: contain`，居中显示
- [x] 调整座位位置算法参数（cx/cy/rx/ry）适配新背景
- [x] 移除旧的 CSS 椭圆相关样式代码
- [x] 确保 PotDisplay 在桌面中央正确显示
...
- [x] GamePage 四角添加 CSS radial-gradient 光晕层
- [x] 颜色对应设计稿：左上绿光、右下红光、微弱紫色点缀
- [x] 光晕层设为 `pointer-events-none`，不影响交互
...
- [x] 头像尺寸从 48px 放大到 56px
- [x] 添加 conic-gradient 霓虹光环（基于玩家颜色）
- [x] 当前行动玩家的光环增加脉冲动画
- [x] 简化视觉噪音：减少不必要的边框和阴影层
...
- [x] 按钮改为胶囊形状（`rounded-full`）
- [x] 深色半透明背景 + 青色霓虹边框
- [x] 每个按钮左侧添加 lucide 圆形图标容器
  - 看牌 → `Eye`
  - 跟注 → `ArrowDown`
  - 加注 → `TrendingUp`
  - 弃牌 → `X`
...
- [x] 简化/浮动顶部 Header 栏
- [x] Chat 面板默认收起
- [x] 调整 GameLog 位置避免遮挡牌桌
- [x] 整体验证各组件在牌桌背景上的视觉效果
